...
export function upsertProfile(userId = 1, data) {
    ...
    const extraFields = data.extra_fields
        ...
        : (existing?.extra_fields ? JSON.stringify(existing.extra_fields) : null);
...
export function getConversations(userId = 1, limit = 100) {
    return db.prepare(`
...
export function addConversation(userId = 1, role, content, url = null) {
    const result = db.prepare(`
        INSERT INTO conversations (user_id, role, content, url)
        VALUES (?, ?, ?, ?)
    `).run(userId, role, content, url);

    return result.lastInsertRowid;
}

export function clearConversations(userId = 1) {
